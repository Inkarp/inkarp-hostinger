...
const CareersForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // DO NOT set Content-Type for FormData; Axios adds boundary automatically
      const response = await axios.post(
        "https://inkarppersonal.vercel.app/api/careers/submit",
        formDataToSend,
        {
          timeout: 15000, // surface CORS/slow API quickly
          onUploadProgress: (e) => {
            if (!e.total) return;
            const pct = Math.round((e.loaded * 100) / e.total);
            console.log("Upload progress:", pct + "%");
          },
          // withCredentials: true, // only if your API needs cookies
        }
      );
      ...
      if (response.data?.success) {
        ...
        // Clear input values in the native form
        e.target.reset();
      } else {
        setStatus({
          type: "error",
      ...
      if (error.code === "ECONNABORTED") {
        ...
        setStatus({
          type: "error",
          message:
            "Couldn’t reach the server (possibly CORS or network issue). Please try again or contact support.",
        });
      } else {
        setStatus({
          type: "error",
      ...
      >
          ...
          <div>
            ...
            {!isCustomRole ? (
              <select
                ...
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E63946] bg-white"
              >
          ...
          <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
            <div className="space-y-1 text-center">
              ...
              <div className="flex text-sm">
                <label
                  htmlFor="resume"
                  className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2"
                >
        ...
        <div className="flex justify-center">
          <button
            ...
            className={`px-8 py-3 bg-[#E63946] text-white rounded-lg font-medium hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors ${
